...
% Scaling Generative Language Models to 530 Billion Parameters Using Megatron and DeepSpeed
% Megatron-Turing NLG 530B: Scaling Generative Language Models to 530 Billion Parameters Using Megatron and DeepSpeed

% Megatron-Turing NLG 530B: Scaling Generative Language Models to 530 Billion Parameters
% Scaling Generative Language Models to 530 Billion Parameters

%Megatron-Turing NLG 530B: Pre-training and Evaluation of A Non-Sparse Transformer Language Model with 530 Billion Parameters

%\title{Using DeepSpeed and Megatron to Train Megatron-Turing NLG 530B, the World’s Largest and Most Powerful Generative Language Model} 
%\title{Megatron-Turing NLG 530B: A Large-Scale Generative Language Model Pretrained Using DeepSpeed and Megatron}
%\title{Megatron-Turing NLG 530B: Pretraining Large-Scale Generative Language Models Using DeepSpeed and Megatron}
\title{Using DeepSpeed and Megatron to Train Megatron-Turing NLG 530B, A Large-Scale Generative Language Model}

%\author{A\thanks{equal contribution} \thanks{Microsoft} \and B\footnotemark[1] \thanks{NVIDIA} \and C\footnotemark[2] \and D\footnotemark[3] }
...
\begin{abstract}
%\textbf{Model generated abstract: Xia and Zhun}
%\Mo{I don't think this abstract is suitable for a paper} 
%\GEO{I agree; I made some suggestions as overleaf comments. Also, a model-generated abstract is a very cool idea, but the abstract is the most visible and important part of the paper; it's best to hand-write it and add a line: ``for an Abstract written by the model, see the Appendix".}
%\textbf{Xia: rewrote the abstract with MT-NLG's help}
...
%\todo{\GEO{Decide and find+replace across the entire manuscript: ``pretrain" or ``pre-train"?}}
%\textbf{Xia: Add references, address Mohammad and Yu's feedback}
%\todo{\GEO{Decide whether to use ``Capitalized Nouns in Section Titles" or not, and use consistently} }
%\Mo{references are missing}
...
% this sentence seems orthogonal to the trajectory of the introduction - we want to get to "scale" as fast as possible.
%However, even with task-agnostic pretraining, fine-tuning these models still requires task-specific datasets with thousands or tens of thousands of examples.

%Besides improvements to neural network architecture, learning objectives and training data, 
...
%\begin{itemize}
%    \item Comments: Model architecture part of exp %setup?
%    \item We need plan to system results
%    \item Section 2 need to be restructured
%\end{itemize}

%\section{Challenges and Methodology}
%\subsection{Model Architecture}
%Discuss the model size and challenges
%\subsection{Approaches}
%Parallelism and Performance optimizations
%\subsection{Performances Analysis}
%Discuss the hardware used (GPUs, network configurations), achieved FLOPS, disk throughput, overheads from initialization, stuff related to DeepSpeed
...
%Recomputing the activations during backward propagation trades an additional $33\%$ computation cost in exchange for a memory reduction. 
...
%,https://www.deepspeed.ai/tutorials/pipeline/ ---> needs to be moved to references to appear correctly.
As one
...
3D-parallelism implementation. %~\cite{deepspeed3dblog}. --> This reference implies the software stack was created by only the deepspeed team.
...
%we chose to use a sample from OpenWebText2, Wikipedia, and Books3 datasets.  
%, details can be found in Table [TODO tab:classifier-pos]. 
...
%Table [TODO tab:cc-preprocessing] shows the number of documents remaining in each snapshot after each of the preprocessing steps described in this section. 

% TODO tab:classifier-pos
%       5000 /TEE2-lustre/users/bnorick/data/doc_classifier3/parts/books.txt
%     500000 /TEE2-lustre/users/bnorick/data/doc_classifier3/parts/owt2.txt
%     295000 /TEE2-lustre/users/bnorick/data/doc_classifier3/parts/wiki.txt
%     800000 total

% TODO tab:cc-preprocessing
% 2021-04
% Raw
%   3.4 billion web pages or 350 TiB of uncompressed content
% Extracted
%   Incomplete WARCs: 122 / 79840
%   documents: 845,908,543
%   bytes: 3109.168 GiB (3,338,443,219,282)
% Filtered
%   documents: 154,492,540
%   bytes: 595.828 GiB (639,765,103,978)

% 2020-50
% Raw
%   2.64 billion web pages or 270 TiB of uncompressed content
% Extracted
%   Incomplete WARCs: 21 / 72000
%   documents: 647,706,780
%   bytes: 2468.391 GiB (2,650,414,589,342)
% Filtered
%   documents: 119,922,489
%   bytes: 478.277 GiB (513,546,541,768)
...
%unlikely, due to the nature of the text extraction problem, 
...
%529,307,217,920 parameters.
%In our training, we use the architecture of GPT-2 \cite{gpt2-radford2019language}, a left-to-right generative transformer based language model consisting of 529,307,217,920 parameters.
...
%We split the data using a 98:2 ratio for training and validation.
...
%\textbf{Summarize the findings why we selected these values: How many tokens we choose, effects of initialization, why the value of beta2 0.95, we did search and previous works, and empirically decide.}
...
%Therefore, for each validation loss computation, we randomly shuffle the validation dataset and select a different sequence of 16 million consecutive tokens.

The validation cross-entropy loss is %3.19
...
% It is worthwhile to mention that the attention and hidden dropouts were 0.1 while computing the validation losses.
%Note that due to an unintentional configuration issue, the validation results are computed with dropout turned on, implying that we can probably see even lower validation loss with a proper setting.
...
%  \bottomrule
% \end{tabular}
% \caption{LAMBADA zero-shot, one-shot and few-shot accuracy. \ours~outperform GPT-3 across different settings and establishes new SOTA for all 3 settings.}
...
%  \toprule
%  \textbf{Task} & \textbf{Model} &  \textbf{Zero-shot}  &  \textbf{One-shot} &  \textbf{Few-shot} \\
%  \midrule
%  RACE-h & GPT-3	&	45.50	&	45.90	&	46.80	\\
%         & \ours~(ours)  &   \textbf{47.94}   &   \textbf{48.42}   &   \textbf{47.94}   \\
%  \midrule
...
% In order to examine our model's capability to answer common questions directly without the assist of a relevant passage, we also evaluate our model on question answering tasks.
...
%  \bottomrule
% \end{tabular}
% \caption{Commonsense reasoning results on Winogrande, HellaSWAG and PiQA. We generally observe minor gain or even performance dips when moving from zero-shot to one-shot, but would observe significant gains when we move from zero-shot to few-shot settings.}
...
%However, quite different from the trend in ANLI, we see that our model constantly improves as we move from zero-shot to few-shot, with zero-shot performance at around chance-level. More analysis reveal that having diverse examples in the context debiases our model from predicting a certain label all the time on certain subcases.
...
%       & \ours~(ours) & \textbf{51.61}	&	\textbf{60.01}	&	\textbf{73.16}	\\
%  \bottomrule
% \end{tabular}
...
% %HANS dataset is mainly used as an analysis suite for our model (see section 7 for details), nonetheless we report the aggregate dataset-wide results here.
% }
% \label{tab:nli-perf}
% \end{table}
...
% \subsection{Tasks related to MSFT}
% Question generation tasks, etc.
...
% \begin{itemize}
%     \item How good the model is, specially points of failures
%     \item Examine toxicity, bias, etc
%     \item Responsible AI
% \end{itemize}

% \begin{itemize}
%     \item Intro: we analyze the model along multiple dimensions: bias in gender, religion and race; toxicity. Mention that we are following OpenAI
%     \item Gender bias: We perform occupation associativity test with gender. we obtain a list of occupations from "Man is to computer programmer as woman is to homemaker? debiasing word embeddings." and Thomas Manzini paper. We have a list of 323 occupations. We hand craft templates based on Violet Peng's regard paper. We have X templates. We calculate \% of occupations fr which male identifiers have a higher prob over female identifiers. We also calculate average bias score. These are not robust analysis. We do sentiment analysis of the most frequent adjectives used and we see no difference. Note the binary nature of gender. Add OpenAI nos 
%     \item TODO: mention that we only consider the binary gender
%     \item Racial bias: we perform sentiment analysis
%     \item Religious beliefs: we analysis of co-occurring words
%     \item stereoset? -- add those numbers
%     \item realtoxicity? -- no for time being
% \end{itemize}

%We now present a preliminary analysis of the bias of our model, focusing on issues related to gender, ethnicity and religion.
%We analyze our model along the dimensions of bias and toxicity.
%We present a preliminary analysis to encourage research efforts in these directions.
%We focus on understanding the issues of bias related to gender, race and religion.
%Following Brown et al.\ \cite{brown2020language}, we present association tests, co-occurrence analyses and sentiment analyses designed to assess bias along a variety of dimensions.
%As controlling bias and toxicity is required for deployment of large-scale language models, we hope to encourage further research efforts in these directions. This section serves to document and quantify some of the problems relating to bias of the model. As has been shown in other work (for example Weidinger et al.~\cite{deepmindethicalai}), large language models carry many types of bias which would lead to harm if they were deployed directly. The following examination confirms this finding and quantifies it for this particular model.

\subsection{Gender and Occupation Analysis}
%The gender identity of a person and their occupation carries societal biases. 
...
% DW: XXX THEREFORE WHAT? Is this good or bad? How does it compare to, well, anything?
...
%Gopher~\cite{Rae2021Gopher} did not report the average bias score for both male and female combined and rather reported an average bias score of $-0.74$ for occupations that are biased towards men and a score of $0.52$ for the occupations that are biased towards women.
%$65.8$\% of their occupations are biased towards men.
...
%Our number ($0.77$) which is close to $0$ indicates that the probabilities of male and female identifiers were close with male identifiers having higher probability.
...
%The races considered are \textit{Asian}, \textit{Black}, \textit{Indian}, \textit{Latinx}, \textit{Middle Eastern}, and \textit{White}.
...
%For example co-occurrence analysis will treat the following two sentences as same: \textit{The White man was thought of as racist.} and \textit{The White man was thought of as not racist but a liberal man.}
%We should also note that the co-occurrence analysis focus only on surface level words and don't take into account the sentiment or context associated with each adjective. (WE MIGHT WANT TO EXPAND AND MAKE IT MORE POSITIVE?)

% redundant with sentence below
%In Table~\ref{tab:gender_race_adj} we present the top $10$ distinct words that were used with higher frequency for each identifier.
...
% XXX DW so we took out all of the words that were common to both genders?  doesn't this over-emphasize the differences?
%Encouragingly, we note that the top $10$ most frequent adjectives for both the gender identifiers were similar; in Table \ref{tab:gender_race_adj} we only present the most distinct words that had a higher frequency of co-occurring with one gender over the other.
...
%We observe that the model conforms to gender stereotypes that are present in the training data, such as using more adjectives related to appearance for \textit{female} identifiers and using a more diverse set of adjectives for \textit{male} identifiers.
...
%For ethnicity, we qualitatively observe that more positive adjectives are associated with the \emph{White} ethnicity, and that many quite negative adjectives are associated with other ethnicities.
%We note that the model portrays stereotypes associated with each ethnicity as well. Deployment of this model directly would therefore perpetuate negative stereotypes.
...
%We again observe the top $100$ most frequent words that co-occur with each religion.
...
Figure ~\ref{fig:race_sentiment} %\todo{(REMOVE INDIAN, LATINX, MIDDLE EATERN} 
...
%To quantify this, we analyze the sentiment of all the words that co-occur with each of the ethnicities.
%For each word in the generated text, we use SentiWordNet \cite{nltk-sentiwordnet} to measure both positive and negative scores on a scale of $0$ to $100$.
%We average these scores for all words in the generated text.
%Figure~\ref{fig:race_sentiment} shows the average sentiment scores for each of six ethnicities.

%We observe that for the Black ethnicity, the negative sentiment words co-occur with considerable higher proportion, and that correspondingly positive sentiment words co-occur with lower proportion as compared to the other ethnicities.
%The sentiment for Indian, Latinx, and Asian and Middle Eastern ethnicities are comparable to each other.
%The White ethnicity has the highest positive sentiment score overall and has the second highest negative sentiment score.

%Clearly, the bias exhibited by this model is severe and this model should not be deployed directly.
...
%    \toprule
%    Model & LM\_score & stereotype\_score & iCAT score \\
%    \midrule
%    GPT2 \cite{nadeem-etal-2021-stereoset} & 91.11 & 61.93 & 69.37 \\
%    GPT2 medium \cite{nadeem-etal-2021-stereoset} & 92.21 & 62.74 & 68.71 \\
%    GPT2 large \cite{nadeem-etal-2021-stereoset} & 92.49 & 64.26 & 66.12 \\
%    Ours & 93.89 & 68.65 & 58.87 \\
%    \bottomrule
%    \end{tabular}
%    \caption{The overall performance of our model in comparison to other pretrained language models on the development set StereoSet}
%    \label{tab:stereo_set}
%\end{table}

%BCC removing StereoSet due to lack of GPT3 comparison and also the difficulty of explaining this properly. Good results for future work.
%\paragraph{StereoSet Analysis}
%In the experiments described above, we have used a small number of hand-crafted templates; this potentially makes the studies susceptible to personal bias.
%StereoSet \cite{nadeem-etal-2021-stereoset} is a large-scale dataset of natural English sentences designed to measure stereotypical biases in four domains: gender, profession, race, and religion.

%We focus on the intrasentence task which measures bias at the sentence level.
%The task contains a fill-in-the-blank style context sentence describing a target group, and a set of three attributes, which correspond to a stereotype, an antistereotype, and a meaningless option for the blank.

%For this task, there are three primary metrics we use:
%\begin{enumerate}
%    \item The \emph{language modeling score} (LM\_score) is defined as the percentage of instances in which the language model prefers the meaningful over meaningless association (where the meaningful association can correspond to either the stereotype or the anti-stereotype option).

%\item The \emph{stereotype score} is defined as the percentage of samples for which a model prefers a
%stereotypical association over an anti-stereotypical association.
%An ideal language model will achieve a stereotype score of 50 because the model picks equal number of stereotypes and anti-stereotypes \cite{nadeem-etal-2021-stereoset}.

%\item We also measure the \emph{idealized context association test} score (iCAT), which is a blend of its language model score and its stereotype score.  The iCAT score is defined as 
%\begin{equation*}
%iCAT = \texttt{LM\_score} * \frac{\min(\texttt{stereotype\_score}, 100 - \texttt{stereotype\_score})}{50}
%\end{equation*}
%An ideal model will achieve an iCAT score of 100, i.e it will achieve a LM\_score of 100 and a stereotype score of 50, making its iCAT score as 100.
%Hence, the higher the iCAT score the less biased the model is.
%\end{enumerate}

% XXX ? over the development set

%The overall results for our model are presented in Table \ref{tab:stereo_set}.  We also compare to GPT-2.
%These results are in agreement with the observations in \cite{nadeem-etal-2021-stereoset} which state that as the model size increases so does its language modeling capabilities (as reflected by its LM\_score) and its stereotype score.
%The iCAT scores are more dependent on the architecture of the model.  For GPT-style models, the iCAT score generally decreases with increase in model size \cite{nadeem-etal-2021-stereoset}.
%The composition of training corpora also affects these scores.
...
%We have included this section to examine the biases present in our model. Like many other related projects, we find these biases to be pervasive and strong. We do not encourage deploying this model directly. Further research is necessary to mitigate these biases and produce a model that can deployed.

\section{Natural Language Understanding and In-Context Learning}
\label{sec:hans}
% XXX DW I moved this paragraph here, because I think it's important to have a "topic sentence" for this section, as opposed to diving in to the limitations discussion with no context.
...
% TODO: The following paragraph is optional
...
% \todo{Has it been checked whether these examples exist in the training dataset?}
...
% Autoregressive models, GPT-2, GPT-3, Jurassic-1, YUAN 1.0 etc
% Other potential relevant work: T5, M6 (a whole series of M6 models), Switch Transformer, Wudao (hard to find anything to cite)

% Categorization
% Monolithic: GPT-2, GPT-3, T5, Jurassic-1, Yuan 1.0
% MoE: Switch Transformer, M6 series, Wudao
...
%\todo{The rumored WuDao model doesn't seem to have a good citable source} 
...
%\textbf{cite fast MoE} \Zhun{Cited the official Wu Dao 2.0 website (in Chinese) instead}
...
\toprule
\texttt{Context} $\rightarrow$ \lstinputlisting[frame=shadowbox,firstline=1,lastline=8]{code.py} \\
\texttt{Model Output} $\rightarrow$ \lstinputlisting[frame=shadowbox,firstline=9]{code.py} \\
\hline\hline \\
\texttt{Context} $\rightarrow$ \lstinputlisting[frame=shadowbox,firstline=1,lastline=3]{find_fibs.py} \\
\texttt{Model Output} $\rightarrow$ \lstinputlisting[frame=shadowbox,firstline=4]{find_fibs.py} \\
\bottomrule
\end{tabularx}
